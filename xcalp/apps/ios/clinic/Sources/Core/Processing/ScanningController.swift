...
extension ScanningController: ARSessionDelegate {
    ...
    private func processDepthData(_ depthMap: CVPixelBuffer) {
        // Process depth data and update quality metrics
        updateLidarQualityScore(with: depthMap)
    }
    
    private func updateLidarQualityScore(with depthMap: CVPixelBuffer) {
        // Calculate LiDAR quality score based on depth consistency
